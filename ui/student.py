...
def render():
    ...
    with col_chat:
        ...
        problem_solved = bool(st.session_state.get("problem_solved", False))
        b1, b2 = st.columns(2)
...
def _fast_signals_from_turn(
    ...
    hint_count = int(current_hint_level)

    if _is_hint_or_next_step_request(user_text):
        hint_count += 1
    ...
    return {
        ...
        "hints_used": f"{hint_count} support request(s)",
...
def _reply(user_text: str):
    ...
    if is_response_to_current_problem:
        agent_user_text = (
            ...
            f"Student response:\n{user_text}\n\n"
            "Decide whether the student response answers the current problem, asks for help, or needs another scaffold."
        )
    ...
    context_used["learning_state"] = (
        ...
        f"hint level: {current_hint_level}; "
