...
def _delete_auth_cookie():
    """Best-effort deletion for the persistent browser auth cookie."""
    try:
        import extra_streamlit_components as stx

        cookie_manager = stx.CookieManager(key="mathbridge_auth_cookie_manager")
        cookie_manager.delete(_auth_cookie_name())
    except Exception:
        # The app should still clear Streamlit session state even if cookie deletion
        # is unavailable in the current runtime.
        pass


def _clear_app_login_state():
    """Clear app-level and authenticator-level login state."""
    _clear_student_runtime_state()

    keys_to_clear = set(APP_AUTH_KEYS + [
        "username",
        "name",
...
def render_login():
    ...
    if st.session_state.pop("_mathbridge_logout_pending", False):
        _delete_auth_cookie()
        _clear_app_login_state()
        st.info("You are signed out. Please sign in again when you are ready.")
