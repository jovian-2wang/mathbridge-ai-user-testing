...
.mathbridge-account-list {
    display: flex;
    flex-direction: column;
    gap: 8px;
    margin-top: 10px;
}
.mathbridge-account-row {
    border: 1px solid #e5e7eb;
    border-radius: 12px;
    padding: 9px 10px;
    background: #ffffff;
}
.mathbridge-account-row-title {
    font-weight: 800;
    color: #111827;
    margin-bottom: 4px;
}
.mathbridge-account-row-detail {
    color: #4b5563;
    font-size: 0.86rem;
    line-height: 1.45;
}
.mathbridge-account-note {
    border: 1px solid #bfdbfe;
    border-radius: 12px;
    padding: 9px 10px;
    background: #eff6ff;
...
def _preferred_accounts(
    users: dict,
    preferred_usernames: tuple[str, ...],
) -> list[tuple[str, dict]]:
    """Return configured accounts in a stable user-testing order."""
    accounts = []

    for username in preferred_usernames:
        record = users.get(username)
        if isinstance(record, dict):
            accounts.append((username, record))

    return accounts


def _child_detail(record: dict) -> str:
    ...
    return "Linked child: " + ", ".join(child_names)


def _account_lines_html(accounts: list[tuple[str, dict]]) -> str:
    if not accounts:
        return "<span class='mathbridge-muted'>Not configured</span>"

    rows = []
    for username, record in accounts:
        display_name = str(record.get("display_name") or username)
        details = []

        if record.get("memory_file"):
            details.append(f"Memory: <code>{_escape(record['memory_file'])}</code>")

        child_detail = _child_detail(record)
        if child_detail:
            details.append(_escape(child_detail))

        detail_html = ""
        if details:
            detail_html = (
                "<div class='mathbridge-account-row-detail'>"
                + " · ".join(details)
                + "</div>"
            )
        ...
        rows.append(
            f"""
            <div class='mathbridge-account-row'>
              <div class='mathbridge-account-row-title'>{_escape(display_name)}</div>
              <div><strong>Account:</strong> <code>{_escape(username)}</code></div>
              <div><strong>Password:</strong> <code>{_escape(_record_password(record))}</code></div>
              {detail_html}
            </div>
            """
        )

    return "<div class='mathbridge-account-list'>" + "".join(rows) + "</div>"


def _render_test_account_card(
    role_label: str,
    description: str,
    ...
    note_html = ""
    if note:
        note_html = f"<div class='mathbridge-account-note'>{_escape(note)}</div>"
...
def _render_public_testing_info(users: dict):
    ...
    student_accounts = _preferred_accounts(
        users,
        ("alex", "liam", "maya"),
    )
    teacher_accounts = _preferred_accounts(
        users,
        ("teacher_demo",),
    )
    parent_accounts = _preferred_accounts(
        users,
        ("alex_parent", "liam_parent", "maya_parent"),
    )

    if not parent_accounts:
        parent_accounts = _preferred_accounts(users, ("parent_demo",))
    ...
    st.markdown(
        ...
        <section class="mathbridge-testing-panel">
          ...
          <p>
            ...
            The three student accounts are all available for testing and can be
            used freely.
          </p>
        </section>
        """,
    ...
    with c1:
        _render_test_account_card(
            "Student logins",
            "Use any student account to try the tutoring chat and learner profile.",
            student_accounts,
            note="All three student accounts can be used freely for testing.",
        )
    with c2:
        _render_test_account_card(
            "Teacher login",
    ...
    with c3:
        _render_test_account_card(
            "Parent logins",
            "Use a parent account to view the linked child's weekly summary and home practice.",
            parent_accounts,
        )
...
def render_user_panel():
    ...
    with st.sidebar:
        ...
        try:
            ...
            if st.button("Log out", use_container_width=True, key="sidebar_logout_fallback"):
