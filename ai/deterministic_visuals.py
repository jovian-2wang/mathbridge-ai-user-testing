...
def _coordinate_visual(problem: str) -> dict[str, Any] | None:
    ...
    y_max = origin_y - min_axis * spacing

    for i in range(min_axis, max_axis + 1):
        x = origin_x + i * spacing
        y = origin_y - i * spacing
        grid.append(f'<line x1="{x}" y1="{y_min}" x2="{x}" y2="{y_max}" stroke="#edf2f7" />')
        grid.append(f'<line x1="{x_min}" y1="{y}" x2="{x_max}" y2="{y}" stroke="#edf2f7" />')

    plotted = []
    for idx, (px, py) in enumerate(points[:3], start=1):
        x_val = int(px)
        y_val = int(py)

        # The grid is drawn from -6 to 6 on both axes, so boundary points
        # such as (-2, 6) should still be plotted. The old version allowed
        # y only from -5 to 5, which made y=6 disappear.
        if min_axis <= x_val <= max_axis and min_axis <= y_val <= max_axis:
            x = origin_x + x_val * spacing
            y = origin_y - y_val * spacing
            label_x = min(max(x + 12, x_min + 8), x_max - 82)
            label_y = y + 24 if y <= y_min + 20 else y - 10

            plotted.append(f'<circle cx="{x}" cy="{y}" r="7" fill="#dd6b20" stroke="#7c2d12" stroke-width="1.5" />')
            plotted.append(f'<text x="{label_x}" y="{label_y}" font-size="14" font-weight="700" fill="#dd6b20">P{idx} ({_escape(px)}, {_escape(py)})</text>')
  ...
  {''.join(grid)}
  <line x1="{x_min}" y1="{origin_y}" x2="{x_max}" y2="{origin_y}" stroke="#2d3748" stroke-width="2" />
  <line x1="{origin_x}" y1="{y_min}" x2="{origin_x}" y2="{y_max}" stroke="#2d3748" stroke-width="2" />
  <text x="{x_max + 8}" y="{origin_y + 5}" font-size="15" fill="#2d3748">x</text>
  <text x="{origin_x + 7}" y="{y_min - 6}" font-size="15" fill="#2d3748">y</text>
  {''.join(plotted)}
  <text x="360" y="342" text-anchor="middle" font-size="13" fill="#718096">Use the first number for x and the second number for y.</text>
</svg>"""

    return _visual(
        svg=svg,
        caption="A coordinate-plane template for identifying x- and y-values in an ordered pair.",
        reason="Deterministic coordinate-plane visual selected for a coordinate problem.",
